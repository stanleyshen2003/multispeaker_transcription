...
class SettingActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val intent = intent
        if (intent != null) {
            val receivedServerAddress = intent.getStringExtra("serverAddress")
            val receivedServerPort = intent.getIntExtra("serverPort", 8083)
            val receivedLanguage = intent.getStringExtra("language")

            serverAddressEditText.setText(receivedServerAddress)
            serverPortEditText.setText(receivedServerPort.toString())

            val position = languageOptions.indexOf(receivedLanguage)
            if (position != -1) {
